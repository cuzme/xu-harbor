...
// 控制器：一个对象承载 visible/isDestroy/drawerProps/extra/show/hide/destroy
...
const openAdd = () => drawer.show({ props: { title: "新增", size: 420 } });
const openEdit = () =>
  drawer.show({
    props: { title: "编辑", size: 420 },
    extra: { name: "张三", remark: "VIP 客户" },
  });

// 打开动画结束后用 extra 回填（关闭即销毁，每次打开都是全新表单）
const onOpened = () => {
  const extra = drawer.extra.value ?? {};
